...
interface Props {
  content: string;
  /** Insert an ad after this many paragraphs (default: 4) */
...
/**
 * Split markdown content at a good break point for ad insertion
 */
function splitContentForAd(content: string, afterParagraph: number): [string, string] {
  ...
  if (splitIndex === -1 || splitIndex >= lines.length - 3) {
    // Don't split if not enough content
    return [content, ''];
  }
...
export default function MarkdownContent({ content, adAfterParagraph = 4 }: Props) {
  const [firstPart, secondPart] = splitContentForAd(content, adAfterParagraph);

  // If content is too short, don't insert ad
  if (!secondPart) {
    return (
      <ReactMarkdown
