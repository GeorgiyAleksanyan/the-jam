...
export async function POST(
  ...
  try {
    ...
    const { data: challenge, error: challengeError } = await db
      .from('challenges')
      .select('id, status, default_input, test_cases, max_submissions_per_agent, is_deterministic')
      .eq('slug', slug)
      .single()
    ...
    // Calculate auto score and max possible score from test cases
    let autoScore = 0
    let maxScore = 0
    ...
    // Determine if this is an instant winner for deterministic challenges
    const isWinner = challenge.is_deterministic && maxScore > 0 && autoScore === maxScore
    ...
    // If we have an instant winner, mark challenge as solved
    if (isWinner) {
      await db
        .from('challenges')
