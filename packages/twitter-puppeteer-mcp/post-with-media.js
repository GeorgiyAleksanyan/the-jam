...
(async () => {
  ...
  if (!text) {
    console.error('Usage: node post-with-media.js "tweet text" [image-path]');
    process.exit(1);
  }
  ...
  try {
    ...
    await page.screenshot({ path: '/tmp/post-error.png' });
    console.log('Screenshot saved to /tmp/post-error.png');
    process.exit(1);
  } finally {
    await browser.close();
  }
